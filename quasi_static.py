...
PATH_FIT_DB = "./Input/20_04_2015/Debuckling/Ball_shape/results_fit.txt"
PATH_VENTOUSE_DB = "./Input/20_04_2015/Debuckling/Ventouse/results_ventouse.txt"
PATH_FIT_B = "./Input/20_04_2015/Buckling/Ball_shape/"
PATH_VENTOUSE_B= "./Input/20_04_2015/Buckling/Ventouse/"
PATH_FIT_B2 = "./Input/20_04_2015/Buckling/Ball_shape/results_fit.txt"
PATH_VENTOUSE_B2= "./Input/20_04_2015/Buckling/Ventouse/results_ventouse.txt"
...
SINGULAR = True
OUTPUT_QS = './Output/2015_04_20/Debuckling/'
OUTPUT_G  = './Output/2015_04_20/'
DATE = '2015_03_13'
font = {'family' : 'serif',
        'color'  : 'darkred',
        'weight' : 'normal',
...
if __name__ == "__main__":
    if SINGULAR:
        ...
        conv = 1.0
        fit_data_b = read_fit_file(PATH_FIT_B,multiple=sub_pathes_fit)
        ventouse_data_b = read_ventouse_file(PATH_VENTOUSE_B,multiple=sub_pathes_ventouse)
        fit_data_b2 = read_fit_file(PATH_FIT_B2,)
        ventouse_data_b2 = read_ventouse_file(PATH_VENTOUSE_B2,)

        fit_data_db = read_fit_file(PATH_FIT_DB,conversion=conv)
        ventouse_data_db = read_ventouse_file(PATH_VENTOUSE_DB,conversion=conv)
        ...
        quasi_static_b2, dynamic_b2 = parse_data(INDEX_BUCKLING_2,fit_data_b2,ventouse_data_b2)
        # plt.plot(dynamic_b2['t'],dynamic_b2['x'])
        # plt.show()

        quasi_static_db, dynamic_db = parse_data(INDEX_DEBUCKLING,fit_data_db,ventouse_data_db)

        index_resolution_db = np.arange(0 ,quasi_static_db['t'].shape[0],1)
        index_resolution_b2 = np.arange(0 ,quasi_static_b2['t'].shape[0],1)
        ...
        pb = lmfit.Parameters()
        pb.add('x0b',value= 0.103)
        pb.add('alpha', value=1.0)
        mi_bx0p = lmfit.minimize(residLinearX0p,pb,args=(ref_len,quasi_static_b['p']))
        vb = pb.valuesdict()
        reference_length_b = vb['x0b']+vb['alpha']*quasi_static_b['p']
        print pb.valuesdict()
        ref_len2 = quasi_static_db['x'][800:]+((quasi_static_db['external_volume'][800:]*1000*GRAVITY-MASS*GRAVITY)/STIFFNESS)
        pdb = lmfit.Parameters()
        pdb.add('x0b',value= 0.103)
        pdb.add('alpha', value=1.0)
        mi_dbx0p = lmfit.minimize(residLinearX0p,pdb,args=(ref_len2,quasi_static_db['p'][800:]))
        vdb = pdb.valuesdict()
        reference_length_db = vdb['x0b']+vdb['alpha']*quasi_static_db['p']
        plt.plot(quasi_static_b['p'],ref_len,'-ro')
        plt.plot(quasi_static_db['p'][800:],ref_len2,'-bo')
        # plt.show()
        plt.clf()
        reference_length= quasi_static_db['x'][-1]+((quasi_static_db['external_volume'][-1]*1000*GRAVITY-MASS*GRAVITY)/STIFFNESS)
        Xdb_corrected = -(quasi_static_db['x']-reference_length_db)
        Xb_corrected = -(quasi_static_b2['x']-reference_length_b[-1])
        # plt.plot(quasi_static_db['t'],Xdb_corrected,'-b',label='Xdb(t)')
        # plt.plot(quasi_static_b2['t'],Xb_corrected,'-r',label='Xb(t)')
        plt.plot(index_resolution_db,Xdb_corrected,'-b',label='Xdb(t)')
        plt.plot(index_resolution_b2,Xb_corrected,'-r',label='Xb(t)')
        plt.title('Xdb(t) vs Xb(t)')
        plt.xlabel('Pressure(Pa)')
        plt.ylabel(r'$\Delta X$')
        plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
        plt.savefig(OUTPUT_G+'x_dt_'+DATE+'.png', bbox_inches='tight')
        # plt.show()
        plt.clf()
        plt.plot(quasi_static_db['t'][begi:endi],quasi_static_db['p'][begi:endi],'-o',label='X(p)')
        plt.title('X(p)')
        plt.xlabel('Pext (Pa)')
        plt.ylabel(r'$\Delta X$')
        plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
        plt.savefig(OUTPUT_G+'XdbvsXb_dp_'+DATE+'.png', bbox_inches='tight')
        plt.clf()
        A = quasi_static_db['external_volume'][800:]
        B = (1./(1000.0*GRAVITY))*(STIFFNESS*Xdb_corrected[0:800]+MASS*GRAVITY)
        quasi_static_db['external_volume'] = np.concatenate((B,A),axis=0)

        dvm_db,vm0,A_ERROR_V = deltaVmedian(quasi_static_db['external_volume'],sv,quasi_static_db['max_width'],V0 = False)
        dp_db,vi0,A_ERROR_P = deltaP(quasi_static_db['p'],quasi_static_db['external_volume'],sv,quasi_static_db['max_width'],V0 = False)

        dvm_b,vm0,A_ERROR_V = deltaVmedian(quasi_static_b['external_volume'],sv,quasi_static_b['max_width'],V0 = True)
        dp_b,vi0,A_ERROR_P = deltaP(quasi_static_b['p'],quasi_static_b['external_volume'],sv,quasi_static_b['max_width'],V0=True)
        quasi_static_X_before_b = -(quasi_static_b2['x'][0:1444]-reference_length_b[-1]+0.0005)
        quasi_static_X_after_b = -(quasi_static_b2['x'][3000:]-reference_length_db[0])
        dynamic_t = quasi_static_b2['t'][1444:3000]
        V_quasi_statique_bb = (1./(1000.0*GRAVITY))*(STIFFNESS*quasi_static_X_before_b+MASS*GRAVITY)
        V_quasi_statique_ab = (1./(1000.0*GRAVITY))*(STIFFNESS*quasi_static_X_after_b+MASS*GRAVITY)
        dynamic_b2['t'] =dynamic_t-dynamic_t[0]

        V0d = V_quasi_statique_ab[0]
        dV = V_quasi_statique_bb[-1]  - quasi_static_db['external_volume'][0]
        print dV*1000*GRAVITY
        print V0d*1000*GRAVITY
        print quasi_static_db['external_volume'][0]*1000*GRAVITY
        Vf = (0.001153/(1000.0*GRAVITY)) + V_quasi_statique_ab[0]
        dV = (2.027/(1000.0*GRAVITY*100))
        # Vf = (0.001153/(1000.0*GRAVITY))
        # quasi_static_b2['external_volume'] =
        V_dynamic =( Vf+ dV*np.exp(-dynamic_b2['t']/1.770))
        V_b2 = np.concatenate((V_quasi_statique_bb,V_dynamic,V_quasi_statique_ab),axis=0)
        ...
        dvm_b2,vm0,A_ERROR_V = deltaVmedian(V_b2,sv,fit_data_b2['max_width'],V0 = True,givenVolume=vm0)
        dp_b2,vi0,A_ERROR_P = deltaP(ventouse_data_b2['p'],V_b2,sv,fit_data_b2['max_width'],givenVolume=vi0)


        dvm = np.concatenate((dvm_b,dvm_b2,dvm_db),axis=0)
        dp = np.concatenate((dp_b,dp_b2,dp_db),axis=0)
        ...
        # XERR = A_ERROR_P
        j = 10
        lmkjqsdf = np.arange(0,dvm_b2.shape[0],j)
        dvm_b2 = dvm_b2[lmkjqsdf]
        dp_b2 = dp_b2[lmkjqsdf]
        plt.plot(dvm_db,dp_db,'-og',label = 'Expansion')
        plt.plot(dvm_b,dp_b,'-or',label = 'Isotropic')
        plt.plot(dvm_b2,dp_b2,'ob',label = 'Buckling')
        # plt.errorbar(dp[index_resolution], dvm[index_resolution],xerr=XERR[index_resolution], yerr=A_ERROR_V[index_resolution], fmt='o')
        plt.title('Evolution of volume with '+r'$\Delta P$')
        plt.ylabel('Pressure (Pa)')
        plt.xlabel(r'$\Delta V/V$')
        plt.legend()
        plt.savefig(OUTPUT_QS+'dp_dv_150615'+'.svg', bbox_inches='tight')
        plt.show()
        plt.clf()
        ...
        dict_07_04 = {'path_fit':'./Input/07_04_2015/Ball_shape/results_fit.txt','path_ventouse':'./Input/07_04_2015/Ventouse/results_ventouse.txt','name':'07_04_2015','index_buckling':850,'sub_pathes_fit':None,'sub_pathes_ventouse':None}
        ...
        dict_09_04 = {'path_fit':'./Input/09_04_2015/Buckling/Ball_shape/','path_ventouse':'./Input/09_04_2015/Buckling/Ventouse/','name':'09_04_2015','index_buckling':850,'sub_pathes_fit':sub_pathes_fit,'sub_pathes_ventouse':sub_pathes_ventouse}
        ...
        dict_20_04 = {'path_fit':'./Input/20_04_2015/Buckling/Ball_shape/','path_ventouse':'./Input/20_04_2015/Buckling/Ventouse/','name':'20_04_2015','index_buckling':850,'sub_pathes_fit':sub_pathes_fit,'sub_pathes_ventouse':sub_pathes_ventouse}
        list_To_Files = []
        ...
        list_To_Files.append(dict_20_04)

...
def residual_volume(parameters, t, deltav=None, fit=True,far_field=True):
    v = parameters.valuesdict()
    t0 = 0.0
    if not far_field:
        ...
        deltav_theoretical = v['Vinf']- (v['Vinf'] - v['V0'] ) * np.exp(-(t - t0) / v['tau_long'])
...
def residual_r(parameters, t, r=None, fit=True, far_field= True):
    v = parameters.valuesdict()
    t0 = 0.0
    if not far_field:
...
def residual_far_field(parameters, t, yg=None, fit=True):
    if type(parameters) is not dict:
        v = parameters.valuesdict()
    if v['deltaprim'] > 0:
        v['sh'] = (v['q0p'] + v['lamda'] * v['q0']) / v['deltaprim'] ** 0.5
        yg_th_general = np.exp(-v['lamda'] * (t )) * (
        v['q0'] * np.cosh(v['deltaprim'] ** (0.5) * (t )) + v['sh'] * np.sinh(
            v['deltaprim'] ** (0.5) * (t ))) + v['C1'] + v['C2'] * np.exp(-(t) / v['tau_v']) + v['D1'] + v['D2'] * np.exp(-(t) / v['tau_R'])
    elif v['deltaprim'] == 0:
        v['sh'] = v['q0p'] + v['lamda'] * v['q0']
        yg_th_general = np.exp(-v['lamda'] * (t )) * (v['q0'] + v['sh'] * (t )) + v['C1'] + v['C2'] * np.exp(-(t) / v['tau_v']) + v['D1'] + v['D2'] * np.exp(-(t) / v['tau_R'])
    else:
        v['sh'] = (v['q0p'] + v['lamda'] * v['q0']) / (-v['deltaprim']) ** 0.5
        yg_th_general = np.exp(-v['lamda'] * (t )) * (v['q0'] * np.cos((-v['deltaprim']) ** (0.5) * (t )) + v['sh'] * np.sin((-v['deltaprim']) ** (0.5) * (t ))) + v['C1'] + v['C2'] * np.exp(-t / v['tau_v']) + v['D1'] + v['D2'] * np.exp(-t / v['tau_R'])
    if fit:
...
def far_field(filename, time_cut, v0, yg0,ly0, rho, k, g=9.81,mass=0,manip_name="manip",filter=False):
    time_cut +=DELTA_T
    results_dict = read_results_file(filename, time_cut)
    ...
    R = abs((results_dict['Gravity_Y']-results_dict['Ylow']))-0.025
    if filter:
        FILTER_WIN = 11
        print 'filtering\n'
    ...
    volume_params,v_th = volume_fit(time, volume_array_normalized)
    R_params,R_th = r_fit(time, R, volume_params)
    far_field_param, yg_th = far_field_fit(time, yg, volume_params, R_params, rho, k,mass=mass)
...
if __name__ == "__main__":
    ...
    print filename
    rho = 1180
    manip_analyser(filename, (5, 4), rho, mass=m, free_oscillation_flag=False,filter = False)
